  ...
  return (
    <div>
      <h1 style={{display: "flex", marginLeft: "45%"}}>HOME PAGE</h1>
       {userData.map((user) => user.username === "dragonwitch" ? <div key={user.id} className= 'card-container' style={{marginTop:"3%", marginLeft: "25%", marginRight: "-20%"}}>
         <div className='image-container'><img src={user.icon} alt="icon" /></div>
         <h2 style={{borderStyle: "solid", color: "blue"}}>{user.username} {}</h2>
